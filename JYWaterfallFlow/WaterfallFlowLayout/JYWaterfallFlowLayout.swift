...
class JYWaterfallFlowLayout: UICollectionViewLayout {
    ...
    // 当前 section 的头部和上个 section 的尾部的间距，默认为0
    public var spacingWithPreviousSection: CGFloat = 0
    
    /// 数据处理相关
    // 存储 item 的 layoutAttributes 数组
...
extension JYWaterfallFlowLayout {
    /// 为自定义布局做些准备操作，如：初始化变量、所有item的布局样式、计算collectionView的content等（在collectionView重新布局时，总会调用此方法）
...
//        interitemHeightArray.removeAll()
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        // 通过 indexPath，创建一个 UICollectionViewLayoutAttributes (根据indexPath获取item的attributes)
...
//                self.contentScope = contentScope + sectionInset.bottom
            }
    ...
    override func layoutAttributesForSupplementaryView(ofKind elementKind: String, at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if elementKind == UICollectionElementKindSectionHeader {
            ...
            /// 计算 header 的布局
//            //  header 的 y 坐标
////            self.contentScope = contentScope + spacingWithPreviousSection // footer 的上方留白
//            //  设置 header 的 frame
//            layoutAttributes.frame = CGRect(x: 0, y: contentScope, width: headerReferenceSize.width, height: headerReferenceSize.height)
//
//            ///  更新 collectionView 的 content 的值
//            self.contentScope = self.contentScope + headerReferenceSize.height
////            self.contentScope = self.contentScope + sectionInset.top // header 下方留白
            ...
            /// 计算 footer 的布局
//            //  footer 的 y 坐标
////            self.contentScope = contentScope + sectionInset.bottom // footer 的上方留白
//            //  设置 footer 的 frame
//            layoutAttributes.frame = CGRect(x: 0, y: contentScope, width: footerReferenceSize.width, height: footerReferenceSize.height)
//
//            ///  更新 collectionView 的 content 的值
//            self.contentScope = self.contentScope + footerReferenceSize.height
////            self.contentScope = self.contentScope + sectionInset.bottom // footer 的下方留白
...
//            let indexPath = IndexPath(index: i)
    ...
    /*
     *  获取并存储 header/footer 的布局
     *
     *  @param  ofKind:  UICollectionElementKindSectionHeader / UICollectionElementKindSectionFooter
     *  @param  section: 第几个 section 下的所有 item
     *  return  返回该 section 下 header/footer 的布局
     */
    fileprivate func supplementaryViewAttributes(ofKind: String, indexPath: IndexPath) -> [UICollectionViewLayoutAttributes] {
        var saveLayoutAttributesArray: [UICollectionViewLayoutAttributes] = []
        // 获取计算 section 的 header/footer 的布局
        let getSupplementaryViewAttributes = layoutAttributesForSupplementaryView(ofKind: ofKind, at: indexPath)
        if let supplementaryViewAttributes = getSupplementaryViewAttributes {
            // 存储 header/footer 的布局
            saveLayoutAttributesArray.append(supplementaryViewAttributes)
        }
        return saveLayoutAttributesArray
    }
     ...
     *
     *  @param  section: 第几个 section 下的所有 item
     *  return  返回该 section 下所有 item 的布局
     */
    fileprivate func itemAttributes(inSection section: Int) -> [UICollectionViewLayoutAttributes] {
        var saveLayoutAttributesArray: [UICollectionViewLayoutAttributes] = []
        // 获取每个 section 下 item 的个数
        let getItemCount = self.collectionView?.numberOfItems(inSection: section)
        guard let itemCount = getItemCount else { return [] }
        
        // 遍历所有的 item，并获取每个 item 的布局
        for j in 0..<itemCount {
            let indexPath = IndexPath(item: j, section: section)
            let getItemAttributes = layoutAttributesForItem(at: indexPath)
            if let itemAttributes = getItemAttributes {
                // 存储 item 的布局
                saveLayoutAttributesArray.append(itemAttributes)
            }
        }
        return saveLayoutAttributesArray
    }
...
//        }
    ...
    fileprivate func verticalLayoutForItem(_ layoutAttributes: UICollectionViewLayoutAttributes, at indexPath: IndexPath) {
        ...
        } else if delegate != nil && ((delegate?.responds(to: #selector(JYWaterfallFlowLayoutDelegate.collectionView(_:layout:sizeForItemAt:)))) ?? false) {
     ...
     *  横向布局：计算宽度最小的是哪一行
     *
     *  @param  widthArray: 缓存 section 宽度的数组
     *  return  返回最小行的行号和高度值
     */
    fileprivate func minimumLineForSection(widthArray: [CGFloat]) -> (Int, CGFloat) {
        if widthArray.count <= 0 {
            return (0, 0.0)
        }
        // 默认第0列的宽度最小
        var minimumLineNumber = 0
        // 从缓存宽度数组中取出第一个元素，作为最小的那一列的高度
        var minimumLineWidth = widthArray[0]
        // 遍历数组，查找出最小的列号和最小列的高度值
        for i in 1..<widthArray.count {
            let tempMinimumLineWidth = widthArray[i]
            if minimumLineWidth > tempMinimumLineWidth {
                minimumLineWidth = tempMinimumLineWidth
                minimumLineNumber = i
            }
        }
        return (minimumLineNumber, minimumLineWidth)
    }
     ...
     *  return  返回最大行的行号和宽度值
     */
    fileprivate func maximumLineForSection(widthArray: [CGFloat]) -> (Int, CGFloat) {
        if widthArray.count <= 0 {
            return (0, 0.0)
        }
        // 默认第0行的宽度最小
        var maximumLineNumber = 0
        // 从缓存宽度数组中取出第一个元素，作为最小的那一行的宽度
        var minimumLineWidth = widthArray[0]
        // 遍历数组，查找出最大的行号和最大行的高度值
        for i in 1..<widthArray.count {
            let tempMaximumLineWidth = widthArray[i]
            if minimumLineWidth < tempMaximumLineWidth {
                minimumLineWidth = tempMaximumLineWidth
                maximumLineNumber = i
            }
        }
        return (maximumLineNumber, minimumLineWidth)
    }*/
